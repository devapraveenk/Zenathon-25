...
class OrganizationDetails(BaseModel):
    """Complete details of an Organization in a CRM system."""

    organization_name: str = Field(
        ..., min_length=2, max_length=50, description="Name of the Organization."
    )
    website: Optional[str] = Field(
        default="null", description="Email address of the Organization."
    )
    annual_revenue: Optional[int] = Field(
        default=0, description="Revenue of the Organization"
    )
    no_of_employees: Optional[str] = Field(
        default="null", description="Number of the employee in the Organization"
    )
    industry_id: Optional[str] = Field(
        default="null", description="Industry Id of the Organization"
    )
    territory_id: Optional[str] = Field(
        default="null", description="Territory ID of the Organization"
    )
    image: Optional[str] = Field(default="", description="URL of the Image")
...
class CRMAPIClient:
    ...
    def create_organization(self, data: Dict[str, Any]) -> Dict[str, Any]:
        """Create a new organization in the CRM system.

        Args:
            data: Organization data to submit
        ...
        """
        url = f"{self.base_url}mdl=cc8989b5-bb0b-4021-b76d-cf16d8f59841"
...
class LLMManager:
    ...
    def __init__(self, openai_api_key: str, groq_api_key: str):
        ...
        self.parser = JsonOutputParser(pydantic_object=OrganizationDetails)
...
so collect the details from the user query and return it.
Here the image will be a URL so please mind it and parse it.

You must respond with ONLY a valid JSON object in this exact format:
EXAMPLE:

User Query: create Organization as newtest
output: {{"organization_name": "newtest"}} 
...
"DO NOT FORMAT like this: 
    {{
    "annual\\_revenue": 9084,
    "organization\\_name": "new test"
    }}"
...
class DataValidator:
    """Validator for organization data."""
    ...
    def validate_data(data: Dict[str, Any]) -> Dict[str, Any]:
        """Validate data against the OrganizationDetails model.
        ...
        """
        return OrganizationDetails(**data).model_dump()
    ...
    def validate_data_check(
        ...
        Raises:
            ValidationError: If validation fails
        """
        return OrganizationDetails(**{**data, **data_clean})
...
class CreateOrganizationManager:
    """Manager for CRM organizations."""
    ...
    def __init__(self):
        ...
        # API configuration
        self.api_base_url = os.getenv("ORGANIZATION_API_BASE_URL")
    ...
    def prompt_for_mandatory_fields(self):
        ...
        mandatory_fields = ["organization_name"]
        missing_fields = [
            field
    ...
    def process_user_input(self, query: str) -> bool:
        ...
        if query.lower() == "help":
            print("You can ask me to create an organization in the CRM system.")
        ...
        if query.lower() == "submit":
            try:
                ...
                if input("Do you want to submit this data? (y/n): ").lower() == "y":
                    response = self.api_client.create_organization(data=validated_data)
    ...
    def run_interactive_session(self, state):
        """Run an interactive session for organization management."""
        print("CRM Organization Manager - Interactive Session")
        print("Type 'help' for available commands.")
        print("How can I help you create an organization?")
