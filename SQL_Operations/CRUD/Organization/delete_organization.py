...
class DeleteOrganizationManager:
    """A class to manage CRM organizations with API integration and deletion capability"""
    ...
    def __init__(self):
        ...
        self.config = Config(
            ...
            module_id=os.getenv(
                "DELETE_ORGANIZATION_MODEL_ID", "cc8989b5-bb0b-4021-b76d-cf16d8f59841"
            ),
        )
    ...
    def delete_organization(self, name: str) -> bool:
        """
        Delete a organization by first name

        Args:
            name: The first name of the organization to delete

        Returns:
            True if the organization was deleted successfully, False otherwise
        """
        if not name:
        ...
        url = f"{self.config.api_url_base}/api/v1/crm/delete_organization?mdl={self.config.module_id}&&id={organization_id}"
    ...
    def run_interactive_session(self) -> None:
        """Run an interactive session allowing users to search and delete organizations"""
                ...
                try:
                    ...
                    if 1 <= choice <= len(related_names):
                        ...
                        if organization_data:
                            ...
                            confirmation = input(
                                "\nDo you want to Delete this organization? (y/n): "
                            ).lower()
                            if confirmation == "y":
                                success = self.delete_organization(selected_name)
                                if success:
                                    print("organization successfully deleted.")
                                    # Ask if user wants to delete another organization
                                    continue_choice = input(
                                        "Delete another organization? (y/n): "
                                    )
