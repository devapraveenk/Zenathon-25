...
class ContactDetails(BaseModel):
    """Complete details of a contact in a CRM system."""

    first_name: str = Field(
        ..., min_length=2, max_length=50, description="First name of the contact."
    )
    middle_name: Optional[str] = Field(
        default="null", max_length=50, description="Middle name of the contact."
    )
    last_name: str = Field(
        default="",
        min_length=2,
    ...
    full_name: Optional[str] = Field(
        default="null", max_length=150, description="Full name of the contact."
    )
    ...
    address_contact_id: Optional[str] = Field(
        default="null",
        description="Identifier for the address associated with the contact.",
    )
    status: str = Field(
        default="active",
    ...
    designation: Optional[str] = Field(
        default="null", max_length=100, description="Designation of the contact."
    )
    ...
    mobile_code: Optional[str] = Field(
        default="null", description="Mobile code of the contact."
    )
    company_name: Optional[str] = Field(
        default="null", max_length=100, description="Company name of the contact."
    )
    department: Optional[str] = Field(
        default="null", max_length=100, description="Department of the contact."
    )
    industry_id: Optional[str] = Field(
        default="null",
        description="Identifier for the industry associated with the contact.",
    )
    converted: Optional[int] = Field(
        default=0, description="Indicates if the contact has been converted."
    )
    source: Optional[str] = Field(
        default="null", max_length=100, description="Source of the contact."
    )
...
class LLMManager:
    ...
    def __init__(self, openai_api_key: str, groq_api_key: str):
        ...
        self.parser = JsonOutputParser(pydantic_object=ContactDetails)
...
User Query: create contact as ram
...
class DataValidator:
    ...
    def validate_data(data: Dict[str, Any]) -> Dict[str, Any]:
        """Validate data against the ContactDetails model.
        ...
        """
        return ContactDetails(**data).model_dump()
    ...
    def validate_data_check(
        ...
        Raises:
            ValidationError: If validation fails
        """
        return ContactDetails(**{**data, **data_clean})
...
class CreateContactManager:
    """Manager for CRM contacts."""
    ...
    def __init__(self):
        ...
        # API configuration
        self.api_base_url = os.getenv("CONTACT_API_BASE_URL")
    ...
    def prompt_for_mandatory_fields(self):
        ...
        mandatory_fields = ["first_name", "email_id"]
        missing_fields = [
            field
    ...
    def process_user_input(self, query: str) -> bool:
        ...
        if query.lower() == "help":
            print("You can ask me to create a contact in the CRM system.")
        ...
        if query.lower() == "submit":
            try:
                ...
                if input("Do you want to submit this data? (y/n): ").lower() == "y":
                    response = self.api_client.create_contact(data=validated_data)
    ...
    def run_interactive_session(self, state):
        """Run an interactive session for contact management."""
        print("CRM Contact Manager - Interactive Session")
        print("Type 'help' for available commands.")
        print("How can I help you create a contact?")
