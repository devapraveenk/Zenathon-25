...
class ReadContactManager:
    ...
    def get_contact(self, name: str) -> bool:
        """
        get a contact by first name

        Args:
            name: The first name of the contact to get

        Returns:
            True if the contact was getd successfully, False otherwise
        """
        if not name:
        ...
        url = f"{self.config.api_url_base}/api/v1/crm/contact_details_by_id?mdl={self.config.module_id}&&id={contact_id}"
    ...
    def run_interactive_session(self) -> None:
        """Run an interactive session allowing users to search and get contacts"""
                ...
                try:
                    ...
                    if 1 <= choice <= len(related_names):
                        ...
                        if contact_data:
                            ...
                            confirmation = input(
                                "\nDo you want to get and verify this contact details? (y/n): "
                            ).lower()
                            if confirmation == "y":
                                success = self.get_contact(selected_name)
                                if success:
                                    print("Contact successfully verified.")
                                    # Ask if user wants to get another contact
                                    continue_choice = input(
                                        "get another contact? (y/n): "
                                    )
