...
class OpportunityDetails(BaseModel):
    """Complete details of an opportunity in a CRM system."""
    ...
    first_name: str = Field(
        ...
        min_length=2,
        max_length=50,
        description="Name of the Organization or the Deal holder",
    )
    opportunity_name: Optional[str] = Field(
        default="", min_length=2, max_length=100, description="Name of the opportunity."
    )
    opportunity_value: Optional[str] = Field(
        default="null", description="Monetary value of the opportunity."
    )
    status: str = Field(
        default="null", description="Current status of the opportunity."
    )
    recurring: Optional[
        Literal["daily", "weekly", "monthly", "quarterly", "annually"]
    ] = Field(default="daily", description="Recurring frequency of the opportunity.")
...
class CRMAPIClient:
    ...
    def create_lead(self, data: Dict[str, Any], model_id: str) -> Dict[str, Any]:
        """Create a new lead in the CRM system.

        Args:
            data: Lead data to submit
            model_id: Model ID for the lead creation endpoint
        ...
        """
        url = f"{self.base_url}mdl={model_id}"
...
class LLMManager:
    ...
    def __init__(self, openai_api_key: str, groq_api_key: str):
        ...
        self.parser = JsonOutputParser(pydantic_object=OpportunityDetails)
...
User Query: create opportunity as ramma
output: {{"opportunity_name": "ram"}} 
...
class DataValidator:
    """Validator for opportunity data."""
    ...
    def validate_data(data: Dict[str, Any]) -> Dict[str, Any]:
        """Validate data against the OpportunityDetails model.
        ...
        """
        return OpportunityDetails(**data).model_dump()
    ...
    def validate_data_check(
        ...
        Raises:
            ValidationError: If validation fails
        """
        return OpportunityDetails(**{**data, **data_clean})
...
class CreateOpportunityManager:
    """Manager for CRM opportunities."""
    ...
    def __init__(self):
        ...
        # API configuration
        self.api_base_url = os.getenv("OPPORTUNITY_API_BASE_URL")
        self.api_token = os.getenv("API_TOKEN")
        self.api_model_id = os.getenv("OPPORTUNITY_API_MODEL_ID")
    ...
    def prompt_for_mandatory_fields(self):
        ...
        mandatory_fields = ["organization", "first_name"]
        missing_fields = [
            field
        ...
        if query.lower() == "submit":
            try:
                ...
                if input("Do you want to submit this data? (y/n): ").lower() == "y":
                    response = self.api_client.create_lead(
                        data=validated_data, model_id=self.api_model_id
                    )
    ...
    def run_interactive_session(self, state):
        """Run an interactive session for opportunity management."""
        print("CRM Opportunity Manager - Interactive Session")
        print("Type 'help' for available commands.")
        print("How can I help you create an opportunity?")
...
def main():
    ...
    try:
        manager = CreateOpportunityManager()
