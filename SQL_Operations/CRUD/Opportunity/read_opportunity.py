...
class ReadOpportunityManager:
    """A class to manage CRM opportunitys with API integration and Search capability"""
    ...
    def get_opportunity(self, name: str) -> bool:
        """
        get a opportunity by first name

        Args:
            name: The first name of the opportunity to get

        Returns:
            True if the opportunity was get successfully, False otherwise
        """
        if not name:
        ...
        url = f"{self.config.api_url_base}/api/v1/crm/get_opportunity?mdl={self.config.module_id}&&id={opportunity_id}"
        ...
        try:
            ...
            print(f"Error Search opportunity: {str(e)}")
            return False

    def run_interactive_session(self) -> None:
        """Run an interactive session allowing users to search and get opportunitys"""
                ...
                try:
                    ...
                    if 1 <= choice <= len(related_names):
                        ...
                        if opportunity_data:
                            ...
                            confirmation = input(
                                "\nDo you want to get this opportunity? (y/n): "
                            ).lower()
                            if confirmation == "y":
                                success = self.get_opportunity(selected_name)
                                if success:
                                    print("opportunity successfully fetched.")
                                    # Ask if user wants to get another opportunity
                                    continue_choice = input(
                                        "get another opportunity? (y/n): "
                                    )
                                    if continue_choice.lower() != "y":
                                        break
                                else:
                                    print("Failed to get opportunity.")
                            else:
...
def main():
    ...
    try:
        manager = ReadOpportunityManager()
