...
class DeleteTaskManager:
    """A class to manage CRM tasks with API integration and deletion capability"""
    ...
    def delete_task(self, name: str) -> bool:
        """
        Delete a task by first name

        Args:
            name: The first name of the task to delete
        ...
        url = f"{self.config.api_url_base}/api/v1/crm/delete_basic_task?mdl={self.config.module_id}&&id={task_id}"
    ...
    def run_interactive_session(self) -> None:
        """Run an interactive session allowing users to search and delete tasks"""
                ...
                try:
                    ...
                    if 1 <= choice <= len(related_names):
                        ...
                        if task_data:
                            ...
                            if confirmation == "y":
                                success = self.delete_task(selected_name)
                                if success:
                                    print("task successfully deleted.")
                                    # Ask if user wants to delete another task
