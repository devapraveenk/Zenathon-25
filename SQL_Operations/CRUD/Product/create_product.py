...
class ProductDetails(BaseModel):
    """Complete details of a product in a CRM system."""

    product_image: Optional[str] = Field(
        default="null", description="Link or reference to the product image."
    )
    product_name: str = Field(
        ..., min_length=2, max_length=100, description="Name of the product."
    )
    product_type: Optional[str] = Field(
        default="", pattern="^(service|product)$", description="Type of the product."
    )
    description: Optional[str] = Field(
        default="null", description="Description of the product."
    )
    unit_price: Optional[int] = Field(
        default=0, ge=0, description="Unit price of the product."
    )
    unit_of_measure: Optional[str] = Field(
        default="null", description="Unit of measure for the product."
    )
    currency: Optional[str] = Field(
        default="", description="Currency used for the product price."
    )
    tax_rate: Optional[int] = Field(
        default=0, ge=0, description="Tax rate applicable to the product."
    )
    discountable: Optional[str] = Field(
        default="false",
        pattern="^(false|true)$",
...
class CRMAPIClient:
    ...
    def create_product(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        url = f"{self.base_url}mdl=8caca9a9-7c48-4081-9eeb-170009963a5f"
...
class LLMManager:
    ...
    def __init__(self, openai_api_key: str, groq_api_key: str):
        ...
        self.parser = JsonOutputParser(pydantic_object=ProductDetails)
...
User Query: create product as oil
output: {{"product_name": "oil"}} 
...
when using telling about the discount if they are telling as its active then you need to return as "true" if they telling as inactive then you need to return as "false"
...
class DataValidator:
    ...
    def validate_data(data: Dict[str, Any]) -> Dict[str, Any]:
        """Validate data against the ProductDetails model.
        ...
        """
        return ProductDetails(**data).model_dump()
    ...
    def validate_data_check(
        ...
        Raises:
            ValidationError: If validation fails
        """
        return ProductDetails(**{**data, **data_clean})
...
class CreateProductManager:
    ...
    def __init__(self):
        ...
        # API configuration
        self.api_base_url = os.getenv("PRODUCT_API_BASE_URL")
    ...
    def process_user_input(self, query: str) -> bool:
        ...
        if query.lower() == "help":
            print("You can ask me to create a product in the CRM system.")
        ...
        if query.lower() == "submit":
            try:
                ...
                if input("Do you want to submit this data? (y/n): ").lower() == "y":
                    response = self.api_client.create_product(data=validated_data)
    ...
    def run_interactive_session(self, state):
        """Run an interactive session for product management."""
        print("CRM Product Manager - Interactive Session")
        print("Type 'help' for available commands.")
        print("How can I help you create a product?")
