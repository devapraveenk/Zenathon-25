...
class ReadProductManager:
    """A class to manage CRM products with API integration and Search capability"""
    ...
    def get_product(self, name: str) -> bool:
        """
        get a product by first name

        Args:
            name: The first name of the product to get

        Returns:
            True if the product was get successfully, False otherwise
        """
        if not name:
        ...
        url = f"{self.config.api_url_base}/api/v1/crm/get_single_product?mdl={self.config.module_id}&&id={product_id}"
    ...
    def run_interactive_session(self) -> None:
        """Run an interactive session allowing users to search and get products"""
                ...
                try:
                    ...
                    if 1 <= choice <= len(related_names):
                        ...
                        if product_data:
                            ...
                            confirmation = input(
                                "\nDo you want to get this product? (y/n): "
                            ).lower()
                            if confirmation == "y":
                                success = self.get_product(selected_name)
                                if success:
                                    print("product successfully fetched.")
                                    # Ask if user wants to get another product
                                    continue_choice = input(
                                        "get another product? (y/n): "
                                    )
