...
class NoteDetails(BaseModel):
    """Complete details of a note in a CRM system."""

    title: str = Field(
        ..., min_length=2, max_length=100, description="Title of the note."
    )
    content: str = Field(..., description="Content or body of the note.")
    status: Optional[str] = Field(
        default="true", pattern="^(true|false)$", description="Status of the note."
    )
...
class CRMAPIClient:
    ...
    def create_note(self, data: Dict[str, Any]) -> Dict[str, Any]:
        """Create a new note in the CRM system.

        Args:
            data: Note data to submit
...
class LLMManager:
    ...
    def __init__(self, openai_api_key: str, groq_api_key: str):
        ...
        self.parser = JsonOutputParser(pydantic_object=NoteDetails)
...
You are a CRM system that allows users to create notes. You need to collect the details from the user query:
...
User Query: create note with title and content of testnote and this contains just details
output: {{"title": "testnote"
            "content": "this contains just details"
        }} 

ALREADY COLLECTED DATA:
    as of now The Collected data are: {collected_data}
    If the valid data already available dont give '' or None values
...
when using telling about the status if he telling as its active then you need to return as "true" if he telling as inactive then you need to return as "false"
...
class DataValidator:
    ...
    def validate_data(data: Dict[str, Any]) -> Dict[str, Any]:
        """Validate data against the NoteDetails model.
        ...
        """
        return NoteDetails(**data).model_dump()
    ...
    def validate_data_check(
        ...
        Raises:
            ValidationError: If validation fails
        """
        return NoteDetails(**{**data, **data_clean})
...
class CreateNoteManager:
    ...
    def __init__(self):
        ...
        # API configuration
        self.api_base_url = os.getenv("NOTES_API_BASE_URL")
    ...
    def prompt_for_mandatory_fields(self):
        ...
        mandatory_fields = ["title", "content"]
        missing_fields = [
            field
    ...
    def process_user_input(self, query: str) -> bool:
        ...
        if query.lower() == "help":
            print("You can ask me to create a note in the CRM system.")
        ...
        if query.lower() == "submit":
            try:
                ...
                if input("Do you want to submit this data? (y/n): ").lower() == "y":
                    response = self.api_client.create_note(data=validated_data)
            ...
            try:
                ...
                try:
                    DataValidator.validate_data_check(
                        data=self.collected_data, data_clean=cleaned_data
                    )
    ...
    def run_interactive_session(self, state):
        """Run an interactive session for note management."""

        print("CRM Note Manager - Interactive Session")
        print("Type 'help' for available commands.")
        print("How can I help you create a note?")
