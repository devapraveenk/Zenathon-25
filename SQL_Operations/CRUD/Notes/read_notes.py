...
class ReadNoteManager:
    """A class to manage CRM Notes with API integration and Search capability"""
    ...
    def get_Note(self, name: str) -> bool:
        """
        get a Note by first name

        Args:
            name: The first name of the Note to get

        Returns:
            True if the Note was getd successfully, False otherwise
        """
        if not name:
        ...
        url = f"{self.config.api_url_base}/api/v1/crm/get_basic_notes?mdl={self.config.module_id}&&id={Note_id}"
        ...
        try:
            ...
            print(f"Error Searching Note: {str(e)}")
            return False

    def run_interactive_session(self) -> None:
        """Run an interactive session allowing users to search and get Notes"""
                ...
                try:
                    ...
                    if 1 <= choice <= len(related_names):
                        ...
                        if Note_data:
                            ...
                            # Ask if user wants to get another Note
                            continue_choice = input("get another Note? (y/n): ")
