...
class DeleteLeadManager:
    """A class to manage CRM leads with delete operation"""
    ...
    def delete_lead(self, email: str) -> bool:
        """
        Delete a lead by email address

        Args:
            email: The email address of the lead to delete

        Returns:
            True if the lead was deleted successfully, False otherwise
        """
        if not email:
        ...
        url = f"{self.config.api_url_base}/api/v1/crm/delete_lead?mdl={self.config.model_id}&&id={lead_id}"
        ...
        try:
            ...
            print(f"Successfully deleted lead with email: {email}")
            return True
        except requests.exceptions.RequestException as e:
    ...
    def run_interactive_session(self):
        """Run the interactive delete session"""
        print("Starting Lead Manager Interactive Session")

        while True:
            try:
                delete_email = input("Enter the email to delete (or 'exit' to quit): ")

                if delete_email.lower() in ["exit", "cancel", "quit"]:
                ...
                lead_data = self.find_lead_by_email(delete_email)
                if lead_data:
                    print("\n=== Lead to Delete ===")
                    analysis = self.analyze_lead_details(lead_data)
                    print(analysis)

                    confirmation = input("Do you want to delete this lead? (y/n): ")
                    if confirmation.lower() == "y":
                        success = self.delete_lead(delete_email)
                        if success:
                            print("Lead successfully deleted.")
                            # Ask if user wants to delete another lead
                            continue_choice = input("Delete another lead? (y/n): ")
                    ...
                    print(f"No lead found for email '{delete_email}'.")
